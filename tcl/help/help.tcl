...
set helpText(Scid) {<h1>Databases and General Use</h1>
  ...
  <p><i>
 Lumbra's Gigabase <url https://lumbrasgigabase.com/en/>https://lumbrasgigabase.com/en/</url>
  </i></p>
...
<br>
  ...
  <li><a OpeningTrainer>Opening Trainer</a> feature</li>
  <li>Tree <a Tree Training>Opening Trainer</a> (old)</li>
...
set helpText(Searches) {<h1>Searches in Scid</h1>
  ...
  <i>Extra Tags *do not* match normal tags such as White, Event and EventDate</i>.
  </p><p>
  The <b>Flag Options</b> is a hideable section with a combination of Game Properties, User <a Flags>Flags</a>, and Custom Flags.
  At top there are three buttons (Yes, No, Both) which are for convenience and mass-set the User and Custom Flag values.
  One example is given - <i>To find all games with a Custom Flag set, first select "No" to the six custom flag vars, then after executing the search, negating the filter will give you your result.</i>
  </p><p>
...
<green><run ::maint::Open>Maintenance Window</b></run></green> (Control+m).
  ...
  <p>
  Operations include - using <a Flags>Delete (and other User Flags)</a>, <a Maintenance Spellcheck>Spellcheck Names</a>,
...
set helpText(Flags) {<h1>Game Flags</h1>
  ...
  <p>Scid vs. PC also allows quick flag tagging via the <a GameList>Game List</a> context menu. If a custom flag has a description, the gamelist Flag field now shows a lowercase (to distinguish from normal flags) letter instead of a number 1-6 as before.
  </p>
  <p>
  The User Flags are
  </p>
  ...
  <p><footer>Updated: Scid vs. PC 4.27, dec 2025</footer></p>
}
...
set helpText(TB) {<h1>Tablebases</h1>
  ...
  <p>
  Scid vs. PC can use local <b>Nalimov</b> and online <a TB Online>Lichess, Shredder and K4it</a> tablebases.
  </p>
  <p>
...
<url https://web.archive.org/web/20140131015443/http://www.cis.uab.edu/hyatt/crafty/TB/3-4-5/>Bob Hyatt's Ftp (Archive.org)</url>
  ...
  <p>
  Scid vs. PC can use the online tablebases from Lichess, Shredder or K4it.
</p><p>
The <b>Shredder</b> and <b>K4it Nalimov</b> tablebases gives the true value of a position for up to 6 men, showing the depth to mate where relevant.
</p><p>
...
set helpText(Related) {<h1>Links</h1>
  ...
  <br>
 <li>Lumbra's Gigabase <url https://lumbrasgigabase.com/en/>https://lumbrasgigabase.com/en/</url></li>
  ...
  <li>Mouselip's Youtube page <url https://www.youtube.com/@ChessWithMouselip>https://www.youtube.com/@ChessWithMouselip</url></li>
...
<br>
  ...
  <li>Tcl History <url https://tcl.tk/about/history.html>https://tcl.tk/about/history.html</url></li>
...
set helpText(OpeningTrainer) {<h1>Opening Trainer</h1>
  ...
  Databases for the <run ::opening::config><green>Opening Trainer</green></run> must have an appropiate type.
  If openings for white are to be trained, this has
  to be <b>Openings for White</b> (and likewise black and
  both colors). <i>The type can be set via the <a Maintenance>Maintenance
  window</a> by selecting the icon or via the database switchers
  context menu.</i>
  </p>
  <h2>Creating a Repertoire</h2>
  <p>
  <i>The Repertoire Database will be fully loaded at initialisation. For this
  reason, it is important to use a smaller database.</i>
  </p><p>
  ...
  Using <a NAGs>NAGs</a> can be employed to better understand how moves are to be rated.
  </p>
  <p>
  ...
  <h2>General Use</h2>
  <p>
  Open the trainer via Play-<gt>Training-<gt>Opening Trainer.
  The setup dialogue offers some choices about the next training session.
  <ul>
      <li><b>White / Black / Both</b> is used to select the side of the opening to train</li>
      <li><b>Allow only best moves</b> will treat lower rated moves
      as error. The rating of a line is given by NAG values.</li>
      <li><b>Opponent plays best moves</b> Scid will always play
      ...
      <li><b>Don't repeat a game</b> Plays each game only once (unfortunately - even if a game has variations).
      </li>
      <li><b>Only flagged lines</b> Scid will only play lines
      from games flagged as <b>Opening for White</b> or
      <b>Opening for Black</b>.
      </li>
      <li><b>Reset statistics</b> will reset the statistics before the training session.</li>
      <li><b>Max Ply</b> is the maximum depth to which games will be parsed.</li>
  </ul>
  </p>
  <p>
  ...
  values by just enabling these functions. If <b>Show Statistics</b>
  is enabled, the current statistics is shown in the
  color coded fields below the checkbox. Otherwise the overall
  statistics can be accessed by the button <b>Show report</b>
  ...
  <ul>
  <li><b>Green</b> the number of moves played by the user that
  are good moves</li>
  <li><b>Yellow</b> the number of dubious moves played that are
  in the repertoire</li>
  <li><b>Red</b> the number of moves played that are not in the
  repertoire</li>
  <li><b>White</b> the number of times the position was
  encountered</li>
  </ul>
  </p>
  ...
  <ul>
  <li><b>Positions in repertoire</b> the total number of
  positions in the repertoire (all lines in database)</li>
  <li><b>Positions not played</b> the positions that were never
  encountered</li>
  <li><b>Positions played</b> the positions that were played and
  ...
  <li><b>Success</b> the number of good moves made by the player</li>
  <li><b>Dubious moves</b> the number of weak moves made by the
  player</li>
  <li><b>Out of repertoire</b> the number of moves made by the
  player that were not in repertoire</li>
  </ul>
  </p> 
  <p><footer>(Updated: Scid vs. PC 4.27 January 2026)</footer></p>
}
...
<p>
After games are retrieved they are loaded to Scids clipbase
...
<p>
Just above the bottom is another, larger, entry-box. This is Scid vs PC's <b>Console</b> which enables input of Tcl/Tk commands for debugging and advanced use. For example, type 
</p><p><i>
sc_filter reset ; set count [sc_filter count] ; for {set i 1} {$i <lt> $count} {incr i} {if {[sc_flags $i] == {}} {sc_filter remove $i}} ; ::windows::gamelist::Refresh</i></p>
<p>to find all flagged games, or 
</p><p><i>
time {foreach i [array names helpText] {helpWindow $i}}</i></p>
<p>to run a little benchmark. For more information about Tcl, try 
...
<p><footer>Updated: Scid vs. PC 4.27 December 2025</footer></p>
}
