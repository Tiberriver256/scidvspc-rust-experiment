...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// sc_flags
// Usage: sc_flags <gameNum> [verbose]
...
sc_flags (ClientData cd, Tcl_Interp * ti, int argc, const char ** argv)
{
    ...
    if (argc == 2) {
      // We now translate User flags 1-6 to lowercase of the first letter of the description (if set) by default
      char CustomFlagChars[CUSTOM_FLAG_MAX+1];
      db->idx->GetCustomFlagChars (CustomFlagChars);

      uint flag;
      char *s = userFlags; 
      while (*s) {
        flag = (uint)(*s - '1');
        if (flag >= 0 && flag < CUSTOM_FLAG_MAX) {
          if (CustomFlagChars[flag])
            *s = CustomFlagChars[flag];
        }
        s++;
      }
...
sc_game_crosstable (ClientData cd, Tcl_Interp * ti, int argc, const char ** argv)
{
    ...
    if (option == OPT_LATEX) {
        Tcl_AppendResult (ti, "\\documentclass{scrartcl}\n\n",
                          "\\usepackage{booktabs}% provides toprule, midrule, bottomrule\n",
...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~~
// sc_game flag FLAG WHICH [OP]
// sc_game flag <1-6> description
//    sc_game flag takes 2 or 3 extra args (argc == 4|5)
//    2 args - returns the specified flag status for the specified game
//    3 args - the final arg is 0, 1 or invert, and sets/clears the flag for the game/selection
//    Flags that can be specified: delete, user, ...
//    eg
//      sc_game flag <flag> GAMENUM [0|1|invert] ; operates on single game
//      sc_game flag <flag> filter <0|1|invert> ; operates on all filtered games.
//      sc_game flag <flag> all <0|1|invert> ; operates on all games.
//    and for custom flags there are two additional commands
//      sc_game flag <1-6> description
    ...
    while (index < db->numGames  &&  count > 0) {
        if (db->filter->Get(index)) {
            ...
            if (nextMoves && db->treeFilter) {
              ...
              db->idx->GetCustomFlagChars (CustomFlagChars);
              ie->PrintGameInfo (temp, start, index+1, db->nb, formatStr, moveStr->Data(), CustomFlagChars);
            } else {
              db->idx->GetCustomFlagChars (CustomFlagChars);
              ie->PrintGameInfo (temp, start, index+1, db->nb, formatStr, "", CustomFlagChars);
            }
